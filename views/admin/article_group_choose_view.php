    ...
    <script>
        $(function(){
            ...
            $(".btn-select").click(function(){
                $.get("<?=base_url_admin() ?>/article_group/add/<?=$group ?>/" + $(this).attr('data-id'), function( data ) {
                    ...
                    <div class="row-fluid search-bar">
                        <div class="span12">
                            <div class="dataTables_filter input-append">
        						<input type="text" name="search" class="query_field_txt query_field" placeholder="標題" value="<?=$search ?>"/><button type="button" class="btn form-query">查詢</button><a href="<?=base_url_admin() ?>/article_group/choose/<?=$code ?>/<?=$group ?>" class="btn">全部</a>
                            </div>
                        </div>
                    </div>
